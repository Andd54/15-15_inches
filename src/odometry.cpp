...
void odometry() {
  double prev_left = rot_left.get_position();
  double prev_right = rot_right.get_position();
  double theta = 0;
  double prev_theta = 0;
  ...
  double absolute_y = 0;
  while(true) {
    double left = rot_left.get_position();
    double right = rot_right.get_position();
    ...
    if(delta_theta == 0) {
      ...
      local_delta_x = sin(delta_theta / 2) * chassis_wid;
